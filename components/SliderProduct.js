...
function SliderProduct() {
    const products =[
        { src: '/assets/img/product.jpg', alt: '1', name: 'Kênh Động vật', topic:'Chủ đề', sub: '1234', views:'123', link:'https://www.youtube.com/'},
        { src: '/assets/img/product.jpg', alt: '2', name: 'Kênh Động vật' , topic:'Chủ đề', sub: '1234', views:'123', link:'https://www.youtube.com/'},
        { src: '/assets/img/product.jpg', alt: '3', name: 'Kênh Động vật' , topic:'Chủ đề', sub: '1234', views:'123', link:'https://www.youtube.com/'}  
      ];
  return (
  ...
  <div className='flex flex-col relative items-center justify-center text-center'>
    <span className='font-black text-sm lg:text-4xl leading-[-130%]'>Products</span>
  ...
  >
    {products.map((product, index) => (
      <div className="relative h-[350px] lg:h-[400px] xl:h-[500px] flex flex-col group card" key={index}>
        ...
        <div className="absolute inset-0 flex flex-col justify-center items-center text-center text-white p-4 rounded-xl opacity-100 transition-opacity duration-300 ease-in-out group-hover:opacity-0 z-10">
            <h1 className="font-bold text-xl sm:text-lg md:text-xl lg:text-2xl mt-auto">{product.name}</h1>
        </div>
        ...
        <div className="absolute inset-0 flex flex-col text-white p-4 rounded-xl bg-black bg-opacity-50 opacity-0 transition-opacity duration-300 ease-in-out group-hover:opacity-100 z-20">
             <h1 className="font-bold text-lg sm:text-base md:text-lg lg:text-xl mb-2">{product.name}</h1>
            <p className="text-sm sm:text-xs md:text-sm lg:text-base mb-2">Topic: {product.topic}</p>
             <p className="text-sm sm:text-xs md:text-sm lg:text-base mb-2">{product.views} views</p>
            <p className="text-sm sm:text-xs md:text-sm lg:text-base mb-2">{product.sub} subscribe</p>
    ...
    <div className='flex flex-row p-2 bg-[#3f71fe] text-center items-center justify-center rounded-lg'>
      <h1 className='text-sm font-medium w-8/12'>Go to view</h1>
